...
import {
  getCurrentPlan as tsGetCurrentPlan,
  checkDrift as tsCheckDrift,
...
function computeHealth() {
  ...
  // TimeSense snapshot — additive
  const tsPlan = tsGetCurrentPlan ? tsGetCurrentPlan() : null;
  const tsDrift = tsCheckDrift ? tsCheckDrift() : { plan: null, activeBlock: null, minutesBehind: 0, mode: 'ok' };
  ...
  return {
    ...
    timesense: {
      hasPlan: !!tsPlan,
      date: tsPlan?.date ?? null,
      mode: tsPlan?.mode ?? 'normal',
      blocks: tsPlan?.blocks?.length ?? 0,
      aheadOfSchedule: tsPlan?.aheadOfSchedule ?? false,
      behindSchedule: tsPlan?.behindSchedule ?? false,
      activeBlock: tsDrift.activeBlock
        ? {
            id: tsDrift.activeBlock.id,
            label: tsDrift.activeBlock.label,
            status: tsDrift.activeBlock.status,
            start: tsDrift.activeBlock.start,
            end: tsDrift.activeBlock.end,
          }
        : null,
      driftMode: tsDrift.mode,
      minutesBehind: tsDrift.minutesBehind,
    },
