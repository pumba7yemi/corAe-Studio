...
export default function AscendDashboard() {
  ...
  if (loading) {
    return <div className="rounded-xl border p-4 text-sm">Loading Ascendâ€¦</div>;
  }
    ...
    <section className="w-full max-w-3xl mx-auto space-y-6">
      <header className="space-y-1">
        ...
        <p className="text-sm opacity-75">
          {allAligned
            ? "Alignment confirmed. Proceed with todayâ€™s WorkFocusâ„¢."
            : "Alignment pending. Complete your Social Contract in all domains."}
        </p>
      </header>

      <div className="grid grid-cols-3 gap-3">
        <StatusCard label="Home" ok={summary.home} when={summary.latestByDomain.home} href="/home/social-contract" />
        <StatusCard label="Work" ok={summary.work} when={summary.latestByDomain.work} href="/work/social-contract" />
        <StatusCard label="Business" ok={summary.business} when={summary.latestByDomain.business} href="/business/social-contract" />
      </div>

      <nav className="flex flex-wrap gap-2">
        <a className="px-3 py-2 rounded-lg border" href="/work/focus">Open WorkFocusâ„¢</a>
        <a className="px-3 py-2 rounded-lg border" href="/work/pulse">View Pulse</a>
        <a className="px-3 py-2 rounded-lg border" href="/work">Open Work</a>
      </nav>
    </section>
  );
}
