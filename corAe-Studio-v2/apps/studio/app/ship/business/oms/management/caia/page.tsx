...
export default function CAIAHub() {
  const links = [
    ...
    { href: 'diary', label: 'Diary', desc: '3cDTD tasks + Have you?/If not… loops' },
  ];

  return (
    <div className="p-4 space-y-4">
      <h1 className="text-2xl font-bold">CAIA — Executive Assistant</h1>
      <p className="text-sm opacity-70">AI-backed executive assistant embedded in OMS.</p>
      <div className="grid gap-3 sm:grid-cols-2">
        {links.map(l => (
          <Link
            key={l.href}
            href={`/ship/business/oms/management/caia/${l.href}`}
