...
//
// Output shape:
// { ok:true, date, results:[{ dealId, status:"created"|"skipped"|"error", reason?, order?, dryRun? }] }
...
async function launchOne(dealId: string, targetDate: Date, windowHours: number, checkOnly: boolean) {
  ...
  if (!bdo) return { status: "skipped" as const, reason: "no_bdo_equals" };

  // 2) confirm
  const hash16 = bdo.snap.hash.slice(0,16);
  const confirm = await loadBdoConfirm(dealId, hash16);
  if (!confirm) return { status: "skipped" as const, reason: "no_bdo_confirm" };
  ...
  if (!sched.needOrder) {
    return { status:"skipped" as const, reason:`window:${sched.reason}` };
  }

  // 4) retail purchase need
  ...
  if (demandType === "RETAIL_SALES" && !prep.needPurchase) {
    return { status:"skipped" as const, reason:"retail_no_purchase_need" };
  }
...
export async function GET() {
  return NextResponse.json({
    ok: true,
    info: "POST this route with { deals:[dealId...], dateISO?, windowHours?, checkOnly? } to batch-launch BDO orders.",
    expects: {
      deals: "string[] (required)",
      dateISO: "YYYY-MM-DD (optional; defaults to today UTC)",
      windowHours: "number (optional; default 12)",
      checkOnly: "boolean (optional; previews only)",
    },
  });
}
...
export async function POST(req: Request) {
  try {
    ...
    const deals = Array.isArray(body?.deals) ? body!.deals.filter(Boolean) : [];
    if (deals.length === 0) return NextResponse.json({ ok:false, error:"deals[] required" }, { status:400 });
    ...
    const results: Array<{
      dealId: string;
      status: "created" | "skipped" | "error" | "preview";
      reason?: string;
      order?: any;
      auditFile?: string;
      dryRun?: boolean;
    ...
    for (const dealId of deals) {
      try {
        const r = await launchOne(dealId, targetDate, windowHours, checkOnly);
        results.push({ dealId, ...(r as any) });
      } catch (e: any) {
        results.push({ dealId, status:"error", reason: e?.message ?? "unknown_error" });
      }
    }
