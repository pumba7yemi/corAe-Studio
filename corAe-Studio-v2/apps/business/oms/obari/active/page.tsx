// apps/studio/app/business/oms/obari/active/page.tsx
"use client";

/**
 * OBARI â€” Active (Stage 4)
 * - If ?dealId=â€¦ is present, shows a banner that the deal is Active (Pricelock + SLA attached)
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const mod = (await import("@/app/api/business/oms/obari/_singletons").catch(() => null)) as
        ...
        const active = list
          ...
          .map((r: any): ActiveRecord => ({
            ...
            order_no: String(r.order_no ?? r.so_no ?? r.po_no ?? "â€”"),
            direction:
        ...
        if (!cancelled) setMsg(`âŒ ${e?.message || "Failed to load active records"}`);
      }
    }
  ...
  return (
    <main className="p-6 space-y-6 flex flex-col min-h-screen text-zinc-100">
      ...
      <header className="stack">
        <h1 className="text-3xl font-bold">OBARI â€” Active</h1>
      ...
      {dealId && (
        <section className="rounded-2xl border border-emerald-900/40 bg-emerald-900/15 p-4 text-sm">
          <p className="text-emerald-200">
            ...
            {" "}Pricelock and SLA attached. Youâ€™re now in <strong>Active</strong>.
          </p>
            ...
            {rows.map((r) => (
              ...
              >
                ...
                <div className="text-xs text-zinc-400">
                  {r.direction.toUpperCase()} â€¢ {r.status} â€¢{" "}
      ...
      <footer className="pt-4 mt-auto">
        <ArrowNav
          backHref="/business/oms/obari/booking"
          nextHref="/business/oms/obari/report"
        >
          Stage 4 â€” Active
        </ArrowNav>
      </footer>
